import Panic, Printf, Bochs, Exports
...
String: cover from Char* {
    ...
    toLower: func -> This {
	length := this length()
    	result := This new(length)
	for (i in 0..length)
		result[i] = this[i] toLower()
	return result
    }
    ...
    substr: func(off: Int, len := -1) -> String {
	length := this length()

	if (off < 0) off = length + off
	if (off >= this length()) return ""
	
	if (len == -1) len = length - off
    ...
    copy: func(src: Char*) {
	clength := 0
	while (src[clength]) clength += 1	

        for (i in 0..clength + 1)
		this[i] = src[i]
    }
}
...
parseInt: static func(str: String) -> UInt {
	n := 0
	len := str length()
	for (i in 0..len) {
		n = n*10 + (str[i] - '0') 
...
operator == (lhs: Char*, rhs: String) -> Bool {
    if ((lhs as UInt32 == 0) || (rhs == null)) {
        return false
    }
...
operator == (lhs: Char*, rhs: Char*) -> Bool {
    if ((lhs as UInt32 == 0) || (rhs as UInt32 == 0)) {
        return false
    }
